...
class VZlog {
  ...
  constructor(api_url, options) {
        ...
        for (let k in options) {
          ...
          if (this._isObject(options[k])) {
            this.options[k] = Object.assign(
              this._deepClone(this.default_options[k]),
              options[k],
            );
          } else {
            // case {a:true,b:true}
    ...
    if (
      ...
      document.addEventListener("scroll", this._listener_scroll, {
        passive: true,
      });
    ...
    if (
      ...
      if (
        !this.options.browser.once_per_user ||
        !localStorage.getItem(this._browser_data_is_submitted_key)
      ) {
    ...
    if (
      ...
      for (let event of this.options.activity.on) {
        ...
        document.addEventListener(event, this._listener_activity, {
          passive: true,
        });
      }

      this._listener_activity_visibility_stopper = () => {
      ...
      this._listener_activity_stopper = () => this._activityStatus(false);
  ...
  _onLocationChange(func) {
    if (!this._onLocationChange_observer) {
      ...
      const mon_selector = this._onLocationChange_watch_el || "head title";

      const target = document.querySelector(mon_selector);

      this._onLocationChange_observer = new MutationObserver(() => {
  ...
  _listenerClick(e) {
    ...
    let filter = this.options.click.on;

    let only_outbound_links = false;
    if (filter === "only_outbound_links") {
      only_outbound_links = true;
      filter = "a";
    }
    if (Array.isArray(filter)) {
      filter = filter.slice();
      const inb_indx = filter.indexOf("only_outbound_links");
      if (inb_indx > -1) {
        only_outbound_links = true;
    ...
    if (el_a) {
      const href = el_a.href;
      const is_inbound = href.indexOf(window.location.origin) > -1;
      if (is_inbound) {
        if (only_outbound_links) {
          return;
        }
    ...
    if (el.className.trim()) {
      params.el.className = el.className.trim();
    }
    if (el.id) {
      params.el.id = el.id;
    }
  ...
  _activityAutoEnd_daemon() {
    this.activity_setInterval_id = setInterval(() => {
      if (
        this._activityStatus() &&
        this._activity.cur.auto_end_at < Date.now()
      ) {
        this._activityStatus(false);
      }
    }, 500);
  ...
  _activityStatus(status) {
    ...
    if (this._debug_activity) {
      console.log(
        "%cActivity",
        "background-color:orange;",
        cur_time,
        this._activity_is_active_now,
        this._activity,
      );
    }
    ...
    if (status === false) {
      ...
      if (this._activity_is_active_now && this._activity.cur.start_time) {
        this._activity.last_transaction =
          cur_time - this._activity.cur.start_time;
    ...
    if (status === true) {
      ...
      const inact_period = this.options.activity.inactivity_period * 1000;
  ...
  _listenerVerticalScroll() {
    ...
    const cur_scroll = this._getVerticalScrollPercent();
    ...
    const bp_status = this._scroll_breakpoints_status;

    const scroll_breakpoints = this.options.scroll.breakpoints;

    const bp_to_send = [];

    for (let bp of scroll_breakpoints) {
      const bp_is_sent = bp in bp_status && bp_status[bp] ? true : false;
      if (!bp_is_sent && cur_scroll > bp) {
        // Measure from the start of the page. Not relevant if user hasn't been started from the zero position, e.g. came through #hash to the middle of the article!
        bp_to_send.push(bp);
  ...
  getPrettyPathToElement(el, return_array = false) {
    ...
    const ret_arr = arr; // delete the first one
  ...
  getPrettyElementName(el) {
    ...
    const el_className = el.className.trim();
    const el_class_str = el_className
      ? "." + el_className.replace(/ +/g, ".")
      : "";

    //const el_siblings = el.parentNode.childNodes;
    const el_siblings = el.parentNode.querySelectorAll(el_tag + el_class_str);
    let sibl_index = 0;
    let el_index = 0;
  ...
  event(ev, params) {
    ...
    if (this._debug) {
      console.log(
        "%cEVENT " + ev,
        "font-size:150%; background-color:yellow; font-weight:bold;",
        params,
      );
    }

    if (this._isObject(ev)) {
      const ev1 = this._deepClone(ev); // we don't want to affect original ev
      if (!Object.prototype.hasOwnProperty.call(ev1, "event")) {
        return;
      }
    ...
    if (window.location.port) {
      obj_to_send.location.port = window.location.port;
    }
    if (window.location.search) {
      obj_to_send.location.search = window.location.search;
    }
    if (window.location.hash) {
      obj_to_send.location.hash = window.location.hash;
    }
  ...
  _getVerticalScrollPercent() {
    ...
    if (this._debug_scroll) {
      console.log({ scroll_position, window_height, document_height });
    }
    ...
    if (
      (document_height / window_height) * 100 <
      this.options.scroll.iflargerthan
    ) {
      return;
    }

    const scroll_percent =
      ((scroll_position + window_height) / document_height) * 100;
    if (this._debug_scroll) {

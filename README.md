...
- It supports browsers released after 2016 (ES6) and works with any frameworks as well as without them. _(Later: ES5/IE11 version)_
...
```javascript
const _VZLog = new VZlog("/path/to/your/api");
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", options);
```
...
const _VZLog = new VZlog("/path/to/your/api", "browser");
// Track only clicks (with default parameters):
const _VZLog = new VZlog("/path/to/your/api", "click");
// Track only scroll (with default parameters):
const _VZLog = new VZlog("/path/to/your/api", "scroll");
// etc.
```
...
const _VZLog = new VZlog("/path/to/your/api", ["click", "scroll"]);
```
...
const _VZLog = new VZlog("/path/to/your/api", { click: true, scroll: true }); // track clicks & scroll (with default parameters)
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", { click: "only_outbound_links" });
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", {
  click: { on: "a,button.class,button[type=submit]" },
});
...
```javascript
const _VZLog=new VZlog('/path/to/your/api',{'click':['on':'only_outbound_links']});
```

You can use `only_outbound_links` in the mix with the other selectors, but they should be set in Array, not in single string:

```javascript
const _VZLog=new VZlog('/path/to/your/api',{'click':['on':['only_outbound_links','span.someclass']]});
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", {
  scroll: { breakpoints: [50, 90], iflargerthan: 150 },
});
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", {
  scroll: { breakpoints: [10, 40, 70, 95] },
});
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", ["activity"]);
```
...
```javascript
const _VZLog = new VZlog("/path/to/your/api", {
  activity: { on: ["pointermove", "pointerdown", "pointermove", "scroll"] },
});
```
